// ─── Z3 · Warn-Bericht Fedlex-Zitatgraph ↔ LexMetrik-Verweiserkennung ────────
//
...
function main(): void {
  if (!existsSync(GRAPH_PFAD)) {
    console.error(`✗ ${GRAPH_PFAD} fehlt — zuerst \`npm run zitatgraph:generieren\`.`);
    process.exit(0);
    return;
  }
  ...
  const inhalt = `${z.join('\n').replace(/\n+$/, '')}\n`;
  writeFileSync(BERICHT_PFAD, inhalt);

  console.log(`Zitatgraph-Warnungen — ${befunde.length}/${graph.erlasse.length} Erlasse verglichen.`);
  console.log(`  ${vergleichbar} vergleichbare Kanten · ${gesamtGedeckt} verlinkt (${quote} %)`);
  console.log(`  A erkannt-nicht-verlinkt: ${gesamtErkannt} (${pct(gesamtErkannt)} %) · B Warnungen: ${gesamtWarn} (${pct(gesamtWarn)} %)`);
  console.log(`  R2 Ziel ausserhalb Korpus: ${gesamtAusserhalb} · R4 eId ohne Eintrag: ${gesamtOhneEId}`);
  console.log(`  Bericht: messwerte/zitatgraph-warnungen.md (${Buffer.byteLength(inhalt)} Bytes)`);
  console.log('  Hinweis: R1 (Fussnoten-Citations) ist nicht herausrechenbar — Bericht, kein Tor.');
}
