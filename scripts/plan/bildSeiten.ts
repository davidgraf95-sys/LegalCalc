...
export function bauPrompt(e: Einheit, info: SchrittInfo | undefined, erledigt?: ReadonlySet<string>): string {
  ...
  const zeilen = [
    ...
    `7. WEITERBAU (Entscheid David 8.8.2026 — «automatisch weiterarbeiten, ohne dass ich es sagen muss»): Ist der Schritt gelandet und die Session noch tragfähig, NICHT abschliessen, sondern direkt weiterbauen. Reihenfolge: (a) nächste offene Position derselben Dach-Checkliste · (b) sonst oberster ready-Schritt GLEICHER Risikoklasse im selben Wirkungsbereich (npm run plan:next + Kollisionsprüfung gegen fremde wip/Worktrees) · (c) nichts Sinnvolles mehr ⇒ regulär abschliessen (Skill bauschritt, Station E). Je Weiterbau: status=wip setzen, volle Sorgfalt (Tore, ggf. Gegenprüfung), eigener Commit mit eigenem Roadmap-Trailer. NIE sortenrein-widrig auf Risikopfade wechseln, und Schluss SPÄTESTENS bevor der Kontext zur Neige geht — lieber sauber landen als einen Schritt anreissen.`,
    ``,
