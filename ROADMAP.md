...
- [ ] **18-FEHLERBUCH · Davids Alltags-Fehlerfunde (stehender Sammel-Schritt)** *(Entscheid David 8.8.2026 — Kleinvieh bündeln statt einzeln durch die volle Maschine)*
  ...
  - [x] **International-Erlasse liegen unter `/gesetze/bund/`** — **gebaut 29.8.2026** (Entscheid David 29.8.2026: ja, mit Redirects). Kanonisch `/gesetze/international/:kuerzel` für die **37** Staatsverträge; Alt-Adresse leitet dauerhaft (Client-Sprung mit Anker+Query, prerenderter Stub mit canonical/noindex). Reproduktion korrigierte zwei Auftrags-Prämissen (§7): `ebene` kennt nur `bund|kanton` — «international» ist ein **Rechtsgebiet**, 0 Erlasse trugen `ebene:'international'`; und die Brotkrume sagte seit Befund 14 bereits «International», falsch war nur die URL. Wurzel-Fix: **eine** Adress-Ableitung (`lib/normtext/erlassAdresse.ts`) statt ~20 Template-Literalen, 7 davon mit fest verdrahtetem `bund`; Tore `src/tests/erlass-adresse.test.ts` + `e2e/international-adresse-b45.e2e.ts`. **Kein `vercel.json`-301** — er ginge nur als handgepflegte 37er-Schlüsselliste (zweite Wahrheit neben dem Register, §5), Herleitung in `adressUmzug.ts`. *(Cowork-Befund 45, 18.8.2026.)*
