...
- [~] **Effizienz-Dauerauftrag (Token/Prozess)** *(`QS-EFFIZIENZ`, stehender Auftrag David 14.8.2026)*
  ...
  **Detail:** [FAHRPLAN-EFFIZIENZ-CHECKLISTE.md](fahrplaene/FAHRPLAN-EFFIZIENZ-CHECKLISTE.md) §1 —
  ...
  - [ ] **Steuerdeckel-Entscheid (Hooks 221 B, check-*.ts ~180 B Luft — wartet auf David)** · ~~**Projektionskette nach main-Merge**~~ erledigt 5.9.2026: `npm run projektionen` (Zähler/Feed/Historie/Manifest/E2E-Shards, seriell, netzfrei) + Landungs-Skill Nachkontrolle 8/9 (Projektionen nachziehen, `npm ci` in Prüf-Worktrees); kein neues Tor, Drift bewachen check:zaehler/feed/historie/datenhaltung/e2e-shards schon (Beleg #694/#695/#689) · ~~**`src/lib/suche/**` nicht im Risiko-Prädikat** (#681, §6.7)~~ Entscheid 5.9.2026 (QS-EFFIZIENZ, §17-Gegengewicht): KEIN Zuwachs — #681 änderte nur `hervorhebung.ts` (Darstellung, `<mark>`-Spannen), 6 von 8 Dateien sind Ranking/Vokabular/Vorschlag («nicht load-bearing» laut eigenem Kopf), kein datierter Vorfall mit falscher Rechtsaussage; Wiedervorlage nur bei Vorfall an `bgeQuery.ts`/`normQuery.ts` (Zitat → Deep-Link), dann als Teilmenge, nicht Ordner. · ~~**`schlankheit:update` nur gezielt**~~ erledigt 5.9.2026: `--update <pfad…>` setzt nur genannte Einträge, ohne Pfad nur Aufräumen und Exit 1 bei neuen Rot-Dateien, jede Änderung laut; Rot-Beweis + 5 Tests (#699) · ~~`ZhStueckFixture` auch in `-runde2/-runde3`-Fixtures dupliziert (§5, #702)~~ erledigt 5.9.2026 (Jules 13b, #715: Basistyp importiert, runde3 per `extends`). · ~~**Paritäts-Tor kennt nur die `check:seriell`-Kette**~~ erledigt 5.9.2026: Gegenrichtung ci.yml → lokal (check:seriell/gate.sh) mit `ALLOWLIST_NUR_CI` (merge-schutz, perf-budget, perf-lighthouse, je Grund), Rot-Beweis mit entferntem check:testtreue (#712) · ~~**Messung: Klasse «Entwurf-Antwort»**~~ erledigt 5.9.2026: Label `entwurf-antwort` (angelegt, auf #707 gesetzt), vierter Topf in `klassierePrs()`, `entwurf_antworten_7d` in JulesMessung (Schema 5, Zeitreihe migriert), Skill/Vorlage/Fahrplan §5 nachgezogen.
