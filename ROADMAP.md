...
- [ ] **`QS-PLAN-SEQ-FRISCHE` · Veraltete Reihenfolge-Vermerke im Plan melden** *(Anlass: Sitzung 13.8.2026 — `W2·5h-GESETZ-UI` trägt seit dem 31.7. `seq-hart: [QS-UI(a Fundament-Pass + b Hierarchie-Pass)]`; dieser Vorbehalt ist seit dem 4.8.2026 eingelöst [`FAHRPLAN-UI-QUALITAET.md` §2.2 wörtlich: «ist damit eingelöst»], der Vermerk steht aber unverändert in der Zeile. Gemessen: `seqHart`/`seqWeich` werden geparst, byte-gleich zurückgeschrieben und angezeigt, aber von `next.ts` **nirgends ausgewertet** — kein Tor kann darum bemerken, dass ein Vermerk hinfällig ist. Wirkung real: die Zeile las sich beim Sichten der Gesetzes-Fläche als Sperre, die es nicht gab, und erzeugte eine falsche Umweg-Empfehlung an David)* — Reine Prüflogik (`Gegenpruefung: n/a`): `check:plan` meldet Vermerke rot, deren genannte ID unbekannt oder `done` ist; Klammer-Zusätze (`QS-UI(8a)`) sind Freitext und werden **ausdrücklich als nicht maschinell prüfbar ausgewiesen** statt still grün (§8/§6.7). Fläche winzig — **3 Vermerke in 2 Schritten**, darum `S` und nur gebündelt mit anderer `scripts/plan`-Arbeit zu nehmen. Im selben Zug den eingelösten Vermerk an `W2·5h-GESETZ-UI` entfernen. **Detail:** [FAHRPLAN-PLAN-STEUERUNG.md](fahrplaene/FAHRPLAN-PLAN-STEUERUNG.md) §Seq-Frische.
  <!-- @meta id: QS-PLAN-SEQ-FRISCHE · status: ready · of: ja · blocker: null · dep: [] · kollision: [scripts/plan, src/tests] · worktree: ja · 26x: nein · groesse: S · fahrplan: fahrplaene/FAHRPLAN-PLAN-STEUERUNG.md -->
