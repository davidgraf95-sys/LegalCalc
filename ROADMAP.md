...
- [ ] **`QS-COCKPIT` · Lagebild wird Steuerpult: Klick «Bau starten» öffnet die Session** *(Auftrag David 8.8.2026 «nur klicken bau und dann wird session geöffnet»)* — `npm run cockpit`: lokaler Mini-Server (nur 127.0.0.1) liefert die Lagebild-Seiten und einen `/bau?id=…`-Endpunkt; Knopf «▶ Bau starten» erscheint nur im Server-Betrieb (auf file:// bleibt Kopieren). Ablauf: ID gegen die baubaren Schritte validieren · Bau-Prompt serverseitig erzeugen (`bauPrompt()`, §5 eine Quelle) · in Temp-Datei schreiben · per osascript neues Terminal-Fenster im Repo mit `claude "$(cat <tmp>)"` öffnen. Sicherheitsregel: kein Text aus dem Request erreicht je eine Shell (nur geprüfte IDs, fester Temp-Pfad); Grenze offenlegen: startet Sessions nur lokal auf Davids Mac.
  <!-- @meta id: QS-COCKPIT · status: ready · of: ja · blocker: null · dep: [] · kollision: [scripts/plan, package.json] · worktree: nein · 26x: nein · groesse: M -->
...
<!-- @queue: QS-SKILL-DIAET, QS-CONFIDENCE-EHRLICH, QS-COCKPIT, QS-AUDIT-VERWEISE, W2·10-UI-NAV, W2·5h-GESETZ-UI, W2·13-KANTONE, W2·6b-MAT-FINMA -->
