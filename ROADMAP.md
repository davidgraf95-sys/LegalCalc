...
- [ ] **Kantonale Gesetze — Daten & Extraktion** *(`W2·13-KANTONE-DATEN`, Aufteilung 8.8.2026, sortenrein)*
  ...
  - [ ] **K-13 · Systematik-Bäume 7 Kantone** *(F6≡F43)* — ZH/GE/VD/TI/SZ/NE/JU fehlen; Quell-Erhebung je Kanton empirisch und browserlos. §1-A.
        *Nachtrag 31.8.2026 (N0b): die sieben fehlenden Bäume kosten jetzt messbar — 34 kantonale Erlasse bleiben ohne `sachgebietKanton`. **ZH ist bestätigt NICHT in `kanton-systematik.json`** (19 Kantone, `grep '"ZH"'` = 0); eine anderslautende Auftrags-Prämisse «ZH-Eintrag seit ZH-4c» ist damit widerlegt (der Kanton mit 14 Wurzel-Ordnern ist BS). Sobald ein Baum erhoben ist, joint sein Kanton ohne Code-Änderung mit. **LU zusätzlich:** Baum vorhanden, aber sein `index` führt Ordinalzahlen statt Systematik-Nummern — 5 Erlasse ohne Feld, bis der Generator dort echte Nummern schlüsselt.*
  ...
  - [ ] **Manifest-Sprache ehrlich + Dubletten** *(Befund Bau W2·13-KANTONE 31.8.2026)* — 37 fr/it-Erlasse als `sprache:'de'` deklariert (nur 2 korrekt ≠ de, §8); mehrere Erlasse doppelt im Manifest (FR-261.16-Notariatstarif, JU-Décret émoluments, TI-Legge tariffa giudiziaria, VS-Notariats-Règlement; dazu GL-Gebührentarif ZGB doppelt als `GL-III%20B_7_1` UND `GL-III%20B%2F7%2F1` — zwei URL-Encodings desselben Erlasses, F12-Prüfung R8.3 1.9.2026). Pipeline-Quelle fixen, nie die Projektion (§5).
