...
- [ ] **`QS-ENTREG-KONFIG` · Drei vorbereitete Konfig-Entlastungen — Anwendung nur durch David** *(Anlass: Ent-Regulierung QS-SELBSTOPT 7.8.2026; Agenten-§0 UND Berechtigungssystem sperren Hook-/Regelwerk-Änderungen für Sessions — zu Recht)* — Drei fertig ausgearbeitete Posten: (a) `gate-stopp.py`-Frequenz — der Stop-Hook misst heute 37,7 s (dokumentiert waren ~7 s) und feuert nach jeder Antwort; Optionen Commit-Grenze / kleinere Schnell-Testmenge / belassen; (b) `tor-schutz.py`-Präzisions-Patch (Fehlalarm auf grep-Suchmuster belegt, Patch beidseitig getestet im Dossier); (c) CLAUDE.md §16 auf Kurzform (~10 → 4 Zeilen Dauerkontext, Wortlaut liegt bereit). **Detail:** [entregulierung-2026-08-07.md](bibliothek/betrieb/entregulierung-2026-08-07.md).
