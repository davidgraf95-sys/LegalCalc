...
- [ ] **`QS-CURRENCY-KANON` · `fza`/`cmr` NICHT-KANONISCH klären und kanonisch nachführen** *(**Risikopfad**)* — Bestandsdefekt auf `main`; erst Ursache klären, dann re-pinnen + regenerieren + §7-Verifikation. **Detail:** [FAHRPLAN-FEDLEX-PORTFOLIO.md](fahrplaene/FAHRPLAN-FEDLEX-PORTFOLIO.md) §17.
  ...
  - [ ] **Nullprobe zuerst** (Nebenfund QS-CURRENCY-TESTS 15.8.): `scripts/fedlex-cache.sh:368` pinnt `fza` bereits auf html-9 (laut §17 kanonisch) — vor dem Bau `npm run check:fedlex-versionen` fahren, Befund vom 2.8. könnte für `fza` erledigt sein; `cmr` ungeprüft. Wurzel-Fix-Kandidat: Wächter «register.json ↔ fedlex-cache.sh ELI-Schlüsselraum deckungsgleich (227/227)» — sonst wird die Kanonik-Dimension still zum No-op (WATCH aus GP #420, nur getestet, nicht geschlossen).
...
- [ ] **`QS-FRIT-DRIFT` · FR/IT-Drift-Wächter Stufe 1 (eId-Mengenvergleich DE/FR/IT)** — gebaut 15.8.2026: `check:frit-drift` vergleicht je 30 Kern-Erlass die eId-Mengen der drei Amtssprachen am gepinnten Stand, im Normen-Monitor verdrahtet. **Erstlauf-Befund:** OR, PatG und BewG weichen in fr/it real ab — ein neu eingeschobener Artikel bekommt dort keinen eigenen eId (Artikel gegenüber eId versetzt) ⇒ **`eId` trägt nicht sprachübergreifend**, Konsequenz für die Zeile «Mehrsprachiger Normvergleich» in `W2·5g-ZEIT`. Dossier: [frit-drift-2026-08-15.md](bibliothek/register/frit-drift-2026-08-15.md). **Detail:** [FAHRPLAN-FEDLEX-PORTFOLIO.md](fahrplaene/FAHRPLAN-FEDLEX-PORTFOLIO.md) §18.1.
  ...
  - [ ] **WARTET AUF DAVID:** die 4 Fedlex-Fundstellen (OR fr/it Art. 219/219a eId-Verschiebung · PatG fr/it Art. 86l ohne eigene eId · BewG fr disp_u1 = andere Schlussbestimmung) dem Fedlex-Betrieb melden? Empfehlung: ja — belegte Fehler in der amtlichen Publikation, Meldung kostet wenig. Belege `bibliothek/register/frit-drift-2026-08-15.md`.
  - [ ] Stufe 2: Abgleich über Artikelnummer statt eId (fängt den BewG-Fall gleiche-eId-anderer-Inhalt); Vollausbau auf alle 227 Pins nach Laufzeit (Stufe 1: ~40 s für 30). Regel für W2·5g-ZEIT: **eId trägt nicht über Sprachen** — Mehrsprachen-Vergleich über Artikelnummer.
...
- [ ] **`QS-MONITOR-ROT` · Normen-Monitor seit ≥5 Wochen rot — Wurzel-Fix** — Aktivierungs-Audit 14.8.2026: `normen-monitor.yml` 5/5 Läufe failure (seit 6.7., Issue #166 offen, 8 rote Läufe in Folge); scheiternde Schritte `check:netz` und LIK-Reihe (BFS O-1.6). Rechtsstand-relevant. DIAGNOSE 14.8. (Session-Befund, Issue #166 beantwortet): Monitor korrekt, Rot ist ECHT — Checkliste: · [ ] LIK-Reihe 2026-05→2026-07 nachziehen (scripts/lik-reihe-generieren.py; amtliche Werte ⇒ Gegenprüfung trotz formal fehlendem Risikopfad-Flag) · [x] 14 nicht-kanonische Fedlex-Pins repariert (#497, 2 Gegenprüfungs-Runden 14/14 SPARQL-rederiviert; PR von 574 auf 10 Dateien entbläht — Automaten-Churn inkl. 115 Kanton-Dateien ist Befund (a2)) · [x] Frische-Automat: gen:historie/check:historie in Kaskade+Prüfliste nachgerüstet (Nullprobe-belegt) · [ ] 10 ESTV-MWST-Snapshot-Drifts aktualisieren (Risikopfad Materialien) · [ ] AIG-Botschaft BOTSCHAFT-2025-3067 nachführen (botschaften-netz rot, Klasse d — materialien:botschaften-Generator; Risikopfad) · [ ] VRV-Vernehmlassung VERN-2026-79 bereinigen (vernehmlassungen-netz rot, Klasse d — Verfahren live nicht mehr gelistet; Risikopfad) · [x] Rest-Sondierung 14.8.: 8 weitere Netz-Tore einzeln GRÜN (caches/zitate/rss-oc/normtext/pdf/pdf-quellen/revisionen/abk) — nur materialien-netz + fedlex-versionen noch offen · [ ] Materialien-System-Befunde 14.8. (aus Korpus-Nachzug, je §17-Wurzel-Fix nötig): (a) `npm run materialien` löscht in DB-losen Worktrees still 11 kanten-Artefakte — Orphan-Bereinigung bei fehlender DB überspringen; (b) Generator-Abgänge ohne Grabstein/Logzeile — Zu-/Abgänge ausgeben, Abgänge bestätigungspflichtig; (c) VERN-Schlüssel erbt mutable Fedlex-Projektnummer (79→78-Umnummerierung belegt) — intrinsische Identität wie bei fga-URIs; (d) botschaften-netz-Stichprobe = 8 feste Keys, blind für Register-Zuwachs (9 neue Erlasse monatelang ungeprüft) — Vollabgleich Grundmenge↔Roh-Dateien; (e) VERN-shas rauschen (stand=Abfragedatum im Hash); (f) Botschaften-Roh ohne ORDER BY — deterministisch sortieren; (h) Fussnoten-Link-Extraktor erzeugt «Link .»-Leerzeichen bei Satzend-Links (TGBV Fn 20/32 belegt, Muster main-weit) — Fix im Extraktor, nie in den Daten; (a2) Frische-Automat fasst bei Bund-Läufen 115 Kanton-Dateien mit Datums-Churn an (Verletzung der eigenen Reset-Invariante cache.sh:31); (g) Generator-Kaskade als EIN Kommando (materialien ⇒ normtext:revisionen ⇒ gen:zaehler — am 14.8. kostete das einzelweise Entdecken zwei CI-Rotläufe auf #499) · [ ] Verfahrens-Gap: Reparatur-Arm (Mo 04:43) vs. Detektions-Arm (Mo 05:17) — Cadence/Reihenfolge entscheiden; check:netz-&&-Kette zeigt nur ersten Befund (eigener deklarierter Schritt, §17).
  ...
  - [ ] **§17-Wurzel-Fix (Gegenprüfungs-Auflage 15.8.):** soft-law-Detektor prüft nur den ToC-Token, nicht das Publikationsdatum — 5 ESTV-Dokumente (MI 02/19, MBI 02/20/25) bekamen neue Kanten-Daten (13.08.2026), ihr register-`stand` blieb aber alt (§5-Zweitwahrheit, Vorbestand). Detektor zusätzlich auf `stand`-Wechsel; Token nur über cipherDisplay-Anker bilden (main-active-Sidebar-Leck der ESTV-App macht ihn volatil, belegt Fetch 1 vs 2/3).
...
- [ ] **11-DESIGN · Design-Wärme & Atmosphäre (Ultracode-Synthese 11.7.)** *(`[OF]`, reine Darstellung/Token-Schicht)*:
  ...
  - [ ] **Design-Qualitäts-Pass Gesetzes-Bereich (Auftrag David 21.8.2026, nach H5)** — fünf PARALLELE Review-Agenten mit getrennten Blickwinkeln (Typografie · Farbe/Themes · Header/Chrome · Layout/Hierarchie · Legal-Tech-Benchmark), je Screenshots hell+dunkel, Desktop+Handy; Massstab «State of the Art Webdesign mit Legal-Tech-Fokus», Dach DESIGN-REGLEMENT; Befunde triagieren (Duplikat-Abgleich Ä-Tabelle FAHRPLAN-LESER-V3), Umsetzung in Bau-Wellen, Geschmacksfragen als Vorlage an David. Spec-Detail: Plan-Datei der Session 21.8. (~/.claude/plans/ich-gebe-dir-gleich-quirky-lobster.md, Abschnitt «Nach H5»).
...
- [ ] **5m-LESER-V3 · Gesetz-Leser V3: Hülle neu, Kern unangetastet** *(Auftrag David 16.8.2026, 19 Positionen; Konzept + Council-Review 16.8.2026)*:
  ...
  - [ ] **D0 · Farb-Vorarbeit vor H1** — DESIGN-D0: Tailwind-Deckkraft-Klassen (`bg-brass-100/70`) erzeugen keine CSS-Regel → Wurzel-Fix + Rot-Beweis (eigener kleiner PR, Vorbedingung H1); DESIGN-D8a für den Split-Partner mitziehen. Kap. 14.
  ...
  - [ ] **S4 Kantons-Probe** — Kantonserlasse rendern unverändert (Fokus Bund, nichts bricht). Kap. 7. ⚠ **Namenskonflikt, 16.8.2026 gemeldet statt stillschweigend aufgelöst:** Fahrplan Kap. 7 führt **S4** als «Sortierung der Suchtreffer auf Erlass-Reihenfolge» und markiert es als mit H2 erledigt; diese Zeile meint etwas anderes. Die **Sortierung ist erledigt** (Vitest an der Sortierfunktion, Mutanten-Test), die **Kantons-Probe nicht** — der H2-Kontaktbogen deckt nur Bund ab. Welcher Inhalt die Kennung S4 behält, entscheidet das nächste Plan-Aufräumen (§5).
...
- [ ] **18-FEHLERBUCH · Davids Alltags-Fehlerfunde (stehender Sammel-Schritt)** *(Entscheid David 8.8.2026 — Kleinvieh bündeln statt einzeln durch die volle Maschine)*
  ...
  - [ ] **Eingebettetes PDF: Ladephase ohne belastbare Messung** — Wortlaut nach Reproduktion korrigiert (29.8.2026): die Fläche ist **hellgrau MIT Ladebalken**, nicht schwarz und nicht ohne Anzeige. Die **Dauer bleibt offen** — sie liess sich mit dem Screenshot-Pfad nicht belastbar messen (die Serie 200 ms → 15 s zeigt ab 1 s ein unverändertes Bild, das ist der Browser-eigene PDF-Viewer, nicht unser Markup). Für eine Zahl braucht es einen echten Browser mit Zeitmessung am `<iframe>`-Load; erst danach ist entscheidbar, ob überhaupt etwas zu tun ist. *(Cowork-Befund 46, 18.8.2026; Wortlaut verifiziert 29.8.2026, Dauer weiterhin unverifiziert.)*
  - [ ] **FR/IT-Band: nicht reproduzierbar wie beschrieben** — Prüfung 29.8.2026: das Band ist ein `<details>`, das sich **sauber schliesst**; weder der «nur durch Aufgeben entfernbar»-Effekt noch der 19-px-Sprung traten auf. **Fundort unklar** — möglicherweise eine andere Stelle, ein anderer Viewport oder ein inzwischen behobener Stand. Zeile bleibt offen, aber als *Suchauftrag nach dem Fundort*, nicht als Fix-Auftrag; ohne neue Repro-Angabe von David ist sie Rückbau-Kandidat (§17). *(Cowork-Befund 48, 18.8.2026; am Prod-Stand nicht reproduzierbar 29.8.2026.)*
  - [ ] **International-Erlasse liegen unter `/gesetze/bund/`** — Routing-Architektur-Frage, David-Entscheid nötig, David-tauglich aufbereiten. *(Cowork-Befund 45, 18.8.2026, unverifiziert — vor Bau reproduzieren.)*
  - [ ] **Kantonswahl: Karte UND Karte/Liste-Umschalter verschwinden nach der Wahl** — «ZH 3» bricht dabei allein um. **Verifiziert 29.8.2026, bewusst NICHT im Fix-Batch gebaut:** der gesamte Kanton-Detailzustand — Kürzelleiste UND der Ort, an dem der Umschalter stehen müsste — liegt in `src/pages/Gesetze.tsx`; `KantonAuswahl.tsx` (wo der Umschalter heute lebt) wird beim Wählen komplett ausgehängt. Diese Datei ist von der offenen Landung PR #565 belegt, darum Sperre. **Direkt nach #565 anschliessen** — dann zusammen mit Befund 12 (SchweizKarte-Overlay, Notiz am Fundort): sobald die Karte im Detailzustand stehen bleibt und `aktiv` durchgereicht wird, wird der latente Befund 12 aktiv. *(Cowork-Befund 41, 18.8.2026; verifiziert 29.8.2026.)*
  - [ ] **Brotkrume Kantonsebene zu flach** — **Prämisse trifft nicht zu (Prüfung 29.8.2026, lex-recherche):** die Leser-Krume trägt bei **keiner** Ebene eine Nummer; sie hat exakt drei Stufen («Gesetze › Bund|Kanton XX › Kürzel», `v3/erlassAnsicht.ts:377-386`). Was auf Prod wie «[SR 220]» aussieht, kommt aus dem Erlass-Kopf DARUNTER (`parts/ErlassLeserKopf.tsx:131-136`), nicht aus der Krume — es gibt also kein Bundes-Vorbild anzugleichen. Der **echte** Unterschied: `zeigeVolltitel()` unterdrückt den Volltitel bei **775/1231 (63 %)** Kantonserlassen, weil dort das Register-Kürzel selbst der Volltitel ist — der Kopf zeigt dann einen langen Token statt «Kürzel + Titel». Die Sigle liegt bereits im `sr`-Feld (ZH → «LS 211.11», 1227/1231 Kantone). Umsetzung = neue Ableitung in `erlassAnsicht.ts` + eigenes Geschwister-Element in `LeserKopf.tsx:204`; **kein Fix-Batch-Posten**: vier e2e-Pins hängen an der Ort-Zone, darunter die bekannte Ä-A4-Breitenfalle (`leser-v3-kopf-buendig.e2e.ts:158-170`). 4/1231 Erlasse ohne `sr` → muss ersatzlos entfallen können (§8). *(Cowork-Befund 43/C8, 18.8.2026; Diagnose verifiziert 29.8.2026.)*
  ...
  - [x] **Browser-Reiter doppelte das Kürzel: «EMRK (EMRK) — LexMetrik»** — reproduziert 29.8.2026 auf `/gesetze/bund/EMRK` und gefixt. Der Kurztitel ist per LEGES-Konvention der Klammer-Inhalt am Ende des Volltitels — bei Staatsverträgen IST das genau das Kürzel. Redundanz-Weiche jetzt als reine Funktion `tabTitel()` in `lib/normtext/erlassKopfText.ts` (dieselbe Regel, die `titelRedundant` längst auf die H1 anwendet, §5), Rot-Beweis in `src/tests/tab-titel-redundanz.test.ts`. *(Fehlerbuch 29.8.2026.)*
  - [ ] **SchweizKarte: aktiver Kanton verliert seinen Rand beim Hover über Nachbarn** — `gezeigt = hover ?? aktiv`, es gibt nur EINEN Overlay-Pfad. **Heute folgenlos**, weil kein Aufrufer `aktiv` übergibt; **scharf**, sobald Befund 41 die Karte im Detailzustand stehen lässt. Notiz steht am Fundort (`src/components/SchweizKarte.tsx`), Auflösung = zweiter dauerhafter Overlay-Pfad für `aktiv` unter dem Hover-Overlay. Gehört ins **Kantonskarten-Paket F1/F2**, nicht in einen UI-Fix-Batch — dort zusammen mit Befund 41 und 49 bauen. *(Cowork-Befund 12; geprüft 29.8.2026.)*
