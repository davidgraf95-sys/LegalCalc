...
- [ ] **18-FEHLERBUCH · Davids Alltags-Fehlerfunde (stehender Sammel-Schritt)** *(Entscheid David 8.8.2026 — Kleinvieh bündeln statt einzeln durch die volle Maschine)*
  ...
  - [x] **Baum-Fokus beim Auto-Zuklappen retten (B8, WCAG 2.4.3):** ✅ PR #483 — `retteFokusVorZuklapp` in `tocAutoZuklappen.ts`, aufgerufen vor dem `flushSync` in `inhalt-hooks.tsx`; sechs Unit-Fälle, Rot-Beweis geführt. *(W2·19-Bug-Check, zurückgestellt.)*
  - [ ] **Perf-Blick auf den langen Artikel-Index (aus PR #483):** der flache Index ist bewusst nicht virtualisiert; seit dem B3-Wegfall trägt SG-3849 607 Zeilen (davon 590 im Anhang-Ast, der bei Anhang-Dominanz aufgeklappt startet), ZH-243 152. Messen, ob das auf schwachen Geräten trägt — sonst Virtualisierung des Index als eigener Schritt (Skill `perf`).
