...
- **Automatik-Gesundheit** *(QS-AUTOMATIK, `[OF]`)*. Läuft unsere Automatik wirklich, und würde sie
  ...
  **Detail:** [FAHRPLAN-BASIS-AUSBAU.md](fahrplaene/FAHRPLAN-BASIS-AUSBAU.md) §1.
  - [ ] **Vercel-Tageslimit durch Branch-Pushes (Vorfall 16.8.2026 abends: 100 Deployments/Tag Free gerissen — jeder Push auf jeden Branch erzeugt ein Deployment, auch «Canceled by Ignored Build Step»; Prod-Deploy von S3 (#540) 24 h blockiert)** — Wurzel-Fix wartet auf David: (a) Vercel-Dashboard Preview-Deployments für das Projekt abschalten (nur Production-Branch), oder (b) Git-Integration trennen und Prod aus CI per `vercel deploy --prebuilt --prod` mit `VERCEL_TOKEN`, oder (c) Pro-Plan. Sofortmassnahme (§0 Ziff. 5): Push nur bei Meilensteinen (Anlage · Abschluss · Nachzug), WIP lokal. Nach Reset deployt Vercel NICHT von selbst — nächster main-Merge löst aus; Prod-Stand-Wächter meldet «hinkt hinter main».
