...
- [ ] **Verwenden statt bauen — risikoarme Fertigteile aus der Fremdquellen-Sichtung 2.9.2026** *(`QS-VERWENDEN`)*
  ...
  - [ ] **V4 JSON-LD vervollständigen** — `legislationDateVersion`/`legislationLegalForce` in `src/lib/seo-detail.ts` aus Konsolidierungsdatum/`inForceStatus` füllen (Geltungsaussage nur, wo der Pin sie kennt; sonst Feld weglassen; Bug-Check #630: Feldname korrekt `legislationDateVersion`, nicht `legislationDate`). Beleg: Rangliste #12.
