...
- **L-3 (M, ✅ von David 10.7. FREIGEGEBEN, nach L-1):** Auto-Default-Umkehr: tiefe Kodifikationen (ZGB/OR) erhalten ihren EINEN Guide auf guideEbene (Umkehr der #161-Politik = **der eigentliche Hebel für Davids Befund**). Referenz-Verdikte + Invariante Z.126/127 in check-linien-kanon.ts deklariert umstellen (§6.3), DESIGN-REGLEMENT-NORMTEXT §4b + linienAufbau-Kopf-Rationale nachziehen. Vorher/Nachher-Screenshots hell/dunkel/mobil im PR.
  ...
  - **↩ ZURÜCKGEZOGEN 12.7.2026 (A28, `#219`):** David prüfte L-3 live an seinen eigenen Erlassen und verwarf die Prämisse — «das mit den linien funktioniert überhaupt nicht» / «also ist überhaupt nicht fördernd für die übersicht» (Wortlaut `docs/ux-audit-2026-07/ANMERKUNGEN-DAVID-2026-07-12.md`). `autoGuide = false` korpusweit; Tor/e2e/Reglement entsprechend zurückgedreht. Doku: `fahrplaene/FAHRPLAN-GESETZES-UX.md` §10.9.
  - **↪ REAKTIVIERT 3.8.2026 (David-Entscheid, `feat/l3-reaktivierung`):** David entschied den Auto-Default erneut zugunsten von L-3 — **in voller Kenntnis der Chronik**: ihm wurde ausdrücklich vorgehalten, dass sein Live-Urteil vom 12.7. der Grund für A28 war und dass A28 der Live-Zustand ist. **A28 ist damit aufgehoben**, die Regel oben (`autoGuide = dichteAmGuide >= DICHTE_MIN`) gilt wieder; `LINIEN_SCHWELLEN` (von QS-OPT #418 als Totcode entfernt, weil A28 sie unbenutzt gelassen hatte) ist wieder verdrahtet. Tor/e2e/Reglement deklariert zurückgestellt (§6.3, Quelle = David-Entscheid). **Zahl-Korrektur gegenüber 11.7.:** der Korpus ist seither von 1144 auf **1416** Sidecars gewachsen und flacher/tiefer verteilt — Auto-Guide AN ist darum **840**, nicht die damaligen 230. Das ist Korpus-Wachstum, keine Regeländerung (identische Regel, identische Schwellen). **A27 bleibt davon unberührt zurückgezogen.** Fällt das Live-Urteil erneut negativ aus, ist der nächste Schritt die Alternativen-Skizze in `FAHRPLAN-GESETZES-UX.md` §10.9 — **nicht** ein weiteres Drehen an `DICHTE_MIN`.
