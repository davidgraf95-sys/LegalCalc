...
Kap. 8/9 Entscheide, Kap. 12/14) und der Kontaktbogen
`docs/ux-audit-2026-07/reader/leser-v3-h4/README.md`. Diese Datei ist der
Momentaufnahme-Zeiger; sie ersetzt `…-handoff-2026-08-17.md` (git-Historie).
...
- **Prüfer:** Bug 2 Befunde, beide **vor** Merge behoben · Ästhetik **7,5/10** ·
  ...
  betrifft V1 und ist zurückgestellt, weil V1 mit H5 fällt).
- **Nachzug A:** Kopf-/Gliederungswege, Steckbrief-Lage Ä89, Icon-Bauform Ä90,
  `data-toc-baum`. **Nachzug B:** Ä81, Schalter-Wortlaut, `AnfangSlot`, Protokoll.
- **A×B-Wechselwirkung, in der Integration gefangen:** `leser-v3-uebersicht` (c3)
  ...
  rot. (c3) zählt jetzt die **Seite**: 1 Seite · 1 Kopf · 0 Box, je Lage.
...
2. **Säuberungs-/H5-PR** («sauberer machen, alles richtig benannt»):
   ...
   - **B6-Wurzelfix** (404 von Netzfehler trennen) ist **Risikopfad**
     `src/lib/normtext/**` → eigener PR mit Gegenprüfung.
...
`src`-Änderung `npm run build`**. Lokal offen (wartet auf David): `git config
--unset core.hooksPath`. CI-Flake `leser-ohne-gliederungslinie:71` Shard 7 →
