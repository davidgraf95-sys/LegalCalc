...
| **H3** | `feat/leser-v3-h3` · `LexMetrik-h3` | `77845c4e1` (gepusht, remote deckungsgleich) | **Bau FERTIG, alle Tore grün** (gate 0, chromium 77/77, leser-v3 59/1 skipped, seo-index 0, perf-budget 0). Panel als Überlagerung (rechts angeschlagenes 22-rem-Blatt auf D, Bottom-Sheet auf H/Pane — Grid-Spalte gestrichen: 70-rem-Wrapper = 1072 px < 1344 nötig, §7-Abweichung mit Rechnung); Nachladen 0 Anfragen bis erstes Öffnen; Kern-Berührung: keine; Prerender trug nie Bezüge → Wächter sichert reale Fassung. Basis = H2b `37159526f` (VOR dem H2b-Nachzug). Vollzugsvermerk Fahrplan ~Z. 694, 66 Bilder `leser-v3-h3/` | Noch **keine Prüfer**: `git rebase --onto <H2b-HEAD 9555f96e8> 37159526f feat/leser-v3-h3` (Konflikte im Rahmen möglich — H2b-Nachzug + H3 «LeserLeisteSheet»/B10-Auflage), Tore, dann drei Prüfer (Briefe `pruefer-briefs.md`), Nachzug (inkl. **B3-V3-Rest aus S1**), PR mit Trailer, nach H2b-Merge landen. Offene H3-Punkte im Vollzugsvermerk (angedockte Spalte → H4-Entscheid; Zähl-Sidecar = Daten-Risikopfad; `leser-kopf-cls-s3` v3@390 ohne Reserve; `leser-kopf-a9` knapp am 5-s-Deckel) |
