...
8. Danach normale Landung (Schritte 0–3) und STRUKTUR-Karte wie bei eigener
   Arbeit — der fremde PR ist kein Grund, die Karte auszulassen. Seit 4.9.2026 fährt CI dieselben zwei Regeln automatisch (Step «Fremd-PR-Tor» im Job «Tore», nur Branches `*-<19 Ziffern>`): Assertion-Diff + keine Datei ausserhalb `src/**` — der lokale Lauf bleibt Pflicht (§14.7, nie die CI-Ausgabe eines Fremden als eigene Prüfung zählen).
