# ARCHIV — ausgelagerte Abschnitte aus `fahrplaene/FAHRPLAN-GESAMTAUFBAU.md`
...
4. **EIN Freigabe-Paket** [DAVID, ~30 Min, reine Ja/Nein-Entscheide, keine Fachprüfung]: (i) E0-Start (Prozess-Freigabe 5), (ii) Fedlex Paket 1 Currency, (iii) Bestätigung 26×-Kette inkl. Slot-Etikett (E3 zuerst, §10.1), (iv) ~~FUNDAMENT-UMBAU Thema B+C Go/No-Go~~ (überholt, s. u.), (v) GmbH-G2-Go (optional, kann warten), (vi) **Turso-Billing dem Grunde nach + Betragsrahmen/Cap** (einziger Turso-Touchpoint; Rückfrage nur bei Cap-Überschreitung), (vii) **Optional-Frage Abnahme-Welle 1: Feb 2027 (Default) oder Vorziehen auf Dez 2026?** **Stand 3.7.2026 (nachgeführt):** (i) ✅ · (ii) ✅ freigegeben+in Arbeit · (iii) ✅ · (v) **«später» (David 3.7.)** · (vi) ✅ (Turso provisioniert+geladen) · (vii) ✅ **Feb 2027 bestätigt** · (viii) ✅ (Dependabot an, dompurify-Fix PR #88 gemergt) — **Nachtrag 31.7.2026 (Archiv-Welle QS-TOK): (iv) ist ÜBERHOLT, kein offener David-Touchpoint mehr** — FUNDAMENT-UMBAU Thema B (Routen-SSoT) und C1–C2 (Abnahme-Dossiers) sind seit 13.6.2026 gebaut (`src/routesManifest.ts`, `src/tests/routenManifest.test.ts`, `scripts/abnahmeDossier.ts`); das Gate wartete auf eine längst getroffene Entscheidung. Der offene Rest (A5/A6, C3/C4, E3/E4, F1–F3/F5) steht als Einzeiler in `ROADMAP.md` → «Strang-Detailpunkte & Hygiene», der Fahrplan selbst liegt in `archiv/FAHRPLAN-FUNDAMENT-UMBAU.md`. **T0b ist damit vollständig abgearbeitet.**
