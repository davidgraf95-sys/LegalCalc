# ARCHIV — ausgelagerte Abschnitte aus `fahrplaene/FAHRPLAN-GESETZES-UX.md`
...
| **K9** | G3 mischt Risiko-Klassen (reine Darstellung + Anhang/Tarif-**Extraktion**) in einem PR — §14 Ziff. 2; «2 Sessions» unterschätzt (annex_* ist Grossvorhaben). | **berechtigt — Blocker** | **G3 gesplittet: G3a** (reine Grundart-Darstellung: Etikett/Präambel/PDF-Rahmen/Live-Karte/Lesespalte — golden byte-gleich-Ziel, `gegenpruefung: n/a`) + **G3b** (Anhang-Block + Tarif-Anhang→echte Tabelle als eigener **Risiko-Pfad**, `check:gegenpruefung`, gekoppelt an `archiv/FAHRPLAN-TARIF-TABELLEN-STUFE2.md` + den `annex_*`-Plan aus `FAHRPLAN-NORMTEXT-DARSTELLUNG.md`, **mehrere Sessions**). |
...
- Tabellen als **echte `<table>`** in `lc-scroll-x`, Bilder/Formeln als `<img>`
  in Bild-Kachel. Verzahnt mit `archiv/FAHRPLAN-TARIF-TABELLEN-STUFE2.md` + dem
...
> Tabelle-Klasse B/C (Extraktion, `archiv/FAHRPLAN-TARIF-TABELLEN-STUFE2.md`).

*Nachtrag 31.7.2026 (ausserhalb des historischen Zitat-Blocks): die Zeile «Offen (G3b)» ist **stale**
— Klasse B und C sind seit 5.7.2026 gebaut, `ROADMAP.md` quittiert «G0–G6 komplett gemergt … inkl.
G3b Schritt 1–3».*
...
| **G3b** | **Risiko-Pfad:** Anhang-Block (③/⑤) + Tarif-Anhang → echte Tabelle. Gekoppelt an `archiv/FAHRPLAN-TARIF-TABELLEN-STUFE2.md` + `annex_*`-Plan. | `ArtikelBody.tsx`, Extraktions-Skripte, `register.ts` | **mehrere Sessions** | **`check:gegenpruefung` zwingend** (Extraktion!); golden neu |
...
### 10.7 · Ausführungsvermerke der §10-Einheiten ✅ (erledigt — Vermerke gebauter §10-Einheiten, Wortlaut: `archiv/FAHRPLAN-GESETZES-UX-erledigt.md`)

*Ausgelagert 15.8.2026 per Fahrplan-§-Diät (`aufraeumen.md` §4b): der volle, unveränderte Wortlaut steht in [`archiv/FAHRPLAN-GESETZES-UX-erledigt.md`](../archiv/FAHRPLAN-GESETZES-UX-erledigt.md). Der §-Anker bleibt hier bestehen — `npm run fahrplan` und `check:plan` Regel 11 lösen weiter auf.*
...
| **A25** | Mehr Farben (Referenzschicht) | **C-1 + C-2 + C-3**, V2 §2 F5 | «gerne auch noch mit mehr farben». **Doktrin: Farbe NUR Referenz-/Verzahnungsschicht (Chips/Badges/Kopf), Normtext-Körper farbfrei** (David-Entscheid). C-1: KantenChip `kategorie`-Prop (norm=brass byte-id / entscheid=slate), StatusBadge Revisions-↻ → warn. C-2: Overline-Farbpunkte + Currency-Tonung (nach U-VERWEIS, im Kopf-PR). C-3: NormChip/Materialien — **✅ GEBAUT 11.7.2026** (`feat/v2-c3`, `13fee95ed`); der frühere Vermerk «DEFER, U-VERWEIS-Kollision» war seit dem U-VERWEIS-Merge am 10.7.2026 (#170, `7f6b9a17b`) überholt und ist am **2.8.2026 §5-geheilt** — Spec-Heimat und massgeblicher Stand: `FAHRPLAN-GESETZESDARSTELLUNG-V2.md` §2/F5, C-3-Zeile («Deferral-Grund weg»). | Golden-neutral (CSS/Token-only). **C-1 SOFORT startbar, kollisionsfrei** (KantenChip/StatusBadge/index.css). C-2/C-3 waren nach U-VERWEIS terminiert — **beide am 11.7.2026 gebaut, Kollisionsauflage erledigt** (Heilung 2.8.2026). Kontrast-Messung als Gate (slate auf paper dunkel 3.31 knapp). |
...
2. **Harte Regel:** KEINE Einheit, die `parts.tsx` oder `inhalt.tsx` berührt, vor dem
   **U-VERWEIS-Merge** (A20/A22/A23/A24/A25-C2 warten — *Wartestand seit 10.7.2026 erledigt,
   s. Ziff. 3*). Precheck vor jeder Einheit:
...
3. **Nach U-VERWEIS:** EIN koordinierter Kopf-PR (A22 K-2 + A23 B-1/B-2 + U-PDF-Slot)
   ...
   **§5-Heilung 2.8.2026:** Die Regel ist **abgearbeitet**, nicht offen — U-VERWEIS (#170) ist am
   **10.7.2026** gemergt (`7f6b9a17b`), A25/C-2 am **11.7.2026** gebaut (`feat/v2-c2`), C-3
   ebenfalls am **11.7.2026** (`feat/v2-c3`, `13fee95ed`). Sie bleibt als Herleitung stehen,
   steuert aber nichts mehr. Massgeblicher Bau-Stand: `FAHRPLAN-GESETZESDARSTELLUNG-V2.md` §2/F5
   (C-1/C-2/C-3-Zeilen) — dort ist die Spec-Heimat, hier nur die Reihenfolge-Ableitung.
