...
// KENNT die Zellenbreite bereits (sie rechnet sie für `lesemassMaxRem` aus
// denselben Grössen) — die Schwelle hängt darum an ihr. Absicht des Fahrplans
...
export interface RahmenBild {
  ...
  /**
   * Ausbaustufe des Satzspiegels (W2·24-R4) — gerechnet aus der Breite, die der
   * Lese-ZELLE nach Gliederungs- und Blatt-Spur bleibt (Herleitung bei
   * `SPUR_MARGINALIE` oben). `'zeile'`, solange nichts gemessen ist: der Leser
   * startet damit in der Ist-Form und baut den Spiegel erst auf, wenn die
   * Breite belegt ist — nie umgekehrt.
   */
...
export function rahmenBild(lage: RahmenLage): RahmenBild {
  ...
  // ── Satzspiegel (R4): wie breit ist die LESE-ZELLE wirklich? ──────────────
  // Dieselben Grössen wie oben, nur diesmal für die Zelle ausgerechnet, die der
  // Text am Ende bekommt. Der Rahmen ist `ruhePx`, ausser das Blatt hat ihn
  // aufgeweitet (`aufweitung` liefert dann `--leser-max-w`); ohne Spaltenlage
  // (Pane, Handy) gibt es keine Spuren und die Zelle IST der Rahmen.
  const rahmenPx = raum
    ? (blattSpur ? Math.max(raum.ruhePx, Math.min(LESER_MAX_REM * rem, raum.raumPx)) : raum.ruhePx)
    : null;
  const zellePx = rahmenPx == null ? null : spaltenLage
    ? rahmenPx
      - (gliederungSpalte ? SPUR_GLIEDERUNG : SPUR_SCHIENE) * rem
      - SPUR_ABSTAND * rem
      - (blattSpur ? (SPUR_BLATT + SPUR_ABSTAND) * rem : 0)
    : rahmenPx;
  const satzspiegel: Satzspiegel = zellePx == null ? 'zeile'
    : zellePx >= SPIEGEL_MIN_VOLL * rem ? 'voll'
    : zellePx >= SPIEGEL_MIN_MARG * rem ? 'marg'
    : 'zeile';
