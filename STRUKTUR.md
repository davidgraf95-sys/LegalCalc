...
- **⚠️ CI auf dem PR ist ROT — und zwar zu Recht: `Browser-Smoke Shard 1/3` (dadurch `Perf-Budget` übersprungen = ebenfalls rot nach `landung`).** Drei Such-Specs laufen nach 2 Retries in `Timeout: 10000ms` mit `Received: 0` — keine falsche Erwartung, keine Flakiness: die Treffer sind auf dem Runner schlicht nicht rechtzeitig da. **Isoliert gemessen:** der clientseitige FlexSearch-Aufbau steigt von **3 153 ms auf 6 143 ms (+95 %)** (Query selbst 2.8 → 4.3 ms, vernachlässigbar). Lokal grün in 4.8–5.9 s; der CI-Runner ist laut eigener Messung in `playwright.config.ts` ~3.9× langsamer → ~19–23 s. **First Paint bleibt unberührt** (Lazy greift, `/gesetze` = 0 Index-Anfragen); betroffen ist nur die Zeit bis zur **ersten** Suche.
- **Bewusst NICHT selbst entschieden** (Auftrag: «sag es mir, statt still zu kürzen»): weder Inhalt gekürzt noch Timeouts hochgesetzt. Drei Wege ohne Inhaltsverlust im PR-Kommentar dokumentiert — (1) gestaffelter Aufbau Bund-zuerst/Kanton-nach (empfohlen, Suche wieder nach ~3 s bereit, Treffer wachsen nach), (2) Timeout-Envelope anheben (billig, macht die Verlangsamung aber unsichtbar), (3) Kanton-Index erst auf Anforderung.
- **Offen für David:** der CI-Entscheid oben · ob ~10 MB gzip bei der ersten Suche tragbar sind · ob «Bund vor Kanton» die gewollte Relevanz-Politik ist. **Befund nebenbei:** `check:suchindex` hängt in **keiner** Gate-Kette (`check:seriell` kennt es nicht) — Index-Drift ist heute ungetestet, ausser jemand ruft es von Hand.
