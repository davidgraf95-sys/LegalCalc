...
- **Beweis:** `npm run gate` voll grün · `check:suchindex` grün · **Golden byte-gleich** · `check:gegenpruefung` grün (kein Risiko-Pfad berührt, daher kein Gegenprüfungs-Protokoll). **Offen für David:** ob der Zuwachs auf ~10 MB gzip für die erste Suche tragbar ist (Alternative wäre Nachladen je Ebene — Inhaltsvollständigkeit ginge dabei nicht verloren, nur die Sofortigkeit), und ob «Bund vor Kanton» die gewollte Relevanz-Politik ist. **Befund nebenbei:** `check:suchindex` hängt in **keiner** Gate-Kette (`check:seriell` kennt es nicht) — Index-Drift ist damit heute ungetestet, ausser jemand ruft es von Hand.
