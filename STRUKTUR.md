...
## Session 24.7.2026 (III) — «bau, run till dry»: QS-TOK-Rest eingeordnet, B24-Split gelandet, M12 im Gegenprüfungs-Loop (Fable-Orchestrator, PR #338 + Worktree `lm-m12`)
...
- **W2·5b/M12 (Randtitel-Verklebung) im Bau, Worktree `lm-m12`:** Root-Cause §7-verifiziert (Fedlex setzt `<br>` in Randtiteln; `reinText()` strippte ersatzlos — `kopf-extrahiere.ts:84` hat das Muster). Bau-Agent lieferte Fix + Tor `check:verklebung` (64 Alt-Treffer rot gezeigt) + Regeneration 227 Bund-Sidecars. **Adversariale Gegenprüfung (unabh. Opus): WIDERLEGT** — Befund 1: `-<br>`-Silbentrennung zerreisst («Adoptions- urlaub», ≥5 Fälle); Befund 2 (vorbestehend): `<b>/<i>`-Inhalts-Strip frisst Titeltext («Cyber pezialistinnen», «( )»). Nachfix mit Korpus-Enumeration + Tor-Erweiterung läuft; danach FRISCHER Prüf-Durchgang. **Nicht quittiert, nicht gemergt** — Tor bleibt bis dahin rot (by design).
